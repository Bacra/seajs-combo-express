...
function writeCache(req, res, file, sender, config) {
	...
	if (fs.existsSync(file)) {
		...
		// static resource
		send(req, file.substr(config.cachePath.length), underscore.extend({}, config, {root: config.cachePath})).pipe(res);
		return false;
	}
	...
	sender.combo.on('readStart', function(lstats, lfile, filePaths) {
			newLstats = lstats;
		})
