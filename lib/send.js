...
proto.send = function() {
	...
	for(var i = 0, len = filePaths.length; i < len; i++)
	{
		...
		if (file.indexOf(config.root) === 0) {
			...
			debug('path overflow, root: %s, file: %s, filepath: %s', root, file, filePaths[i]);
			return false;
		}
	}
...
proto.stream = function(filePaths)
{
	var self = this;

	(new Combo(self._res, filePaths))
